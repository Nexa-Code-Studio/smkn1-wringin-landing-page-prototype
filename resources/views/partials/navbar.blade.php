@php
    $homeJurusanUrl = 'http://127.0.0.1:8002/#konsentrasi-keahlian';
@endphp
...
<nav x-data="{ mobileOpen: false, scrolled: false }"
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex justify-between items-center h-20">
            ...
            <div class="hidden lg:flex space-x-6 items-center">
                ...
                {{-- Profil --}}
                <a href="{{ route('profil') }}" class="text-slate-600 hover:text-brand-600 font-medium transition">Profil</a>

                {{-- Kurikulum Dropdown --}}
                <div class="relative group" x-data="{ open: false }" @mouseenter="open = true" @mouseleave="open = false">
                    <a href="{{ $homeJurusanUrl }}" class="flex items-center gap-1 text-slate-600 hover:text-brand-600 font-medium transition">
                    ...
                    <div x-show="open" 
                        ...
                        <a href="{{ $homeJurusanUrl }}" class="block px-4 py-2 text-sm text-slate-600 hover:bg-brand-50 hover:text-brand-600 transition">Konsentrasi Keahlian</a>
                        <a href="{{ route('pembelajaran') }}" class="block px-4 py-2 text-sm text-slate-600 hover:bg-brand-50 hover:text-brand-600 transition">Pembelajaran</a>
                    </div>
                </div>
                ...
                <div class="relative group" x-data="{ open: false }" @mouseenter="open = true" @mouseleave="open = false">
                    ...
                    <div x-show="open" 
                        ...
                        <a href="{{ route('kemitraan') }}" class="block px-4 py-2 text-sm text-slate-600 hover:bg-brand-50 hover:text-brand-600 transition">Kemitraan</a>
                    </div>
                </div>

                {{-- Berita --}}
                <a href="{{ route('berita') }}" class="text-slate-600 hover:text-brand-600 font-medium transition">Berita</a>
    ...
    <div x-show="mobileOpen" x-transition:enter="transition ease-out duration-200"
        ...
        <div class="px-4 pt-2 pb-6 space-y-1">
            ...
            <a href="{{ route('profil') }}" @click="mobileOpen = false" class="block px-3 py-3 text-slate-600 font-medium hover:bg-slate-50 hover:text-brand-600 rounded-md border-b border-slate-50">Profil</a>
            <a href="{{ $homeJurusanUrl }}" @click="mobileOpen = false" class="block px-3 py-3 text-slate-600 font-medium hover:bg-slate-50 hover:text-brand-600 rounded-md border-b border-slate-50">Kurikulum / Konsentrasi Keahlian</a>
            <a href="{{ route('pembelajaran') }}" @click="mobileOpen = false" class="block px-3 py-3 text-slate-600 font-medium hover:bg-slate-50 hover:text-brand-600 rounded-md border-b border-slate-50">Pembelajaran</a>
            ...
            <div x-data="{ extra: false }">
                ...
                <div class="grid transition-[grid-template-rows] duration-300 ease-in-out" :class="extra ? 'grid-rows-[1fr]' : 'grid-rows-[0fr]'">
                    <div class="overflow-hidden bg-slate-50 rounded-md">
                        ...
                        <a href="{{ route('kemitraan') }}" @click="mobileOpen = false" class="block px-8 py-2.5 text-sm text-slate-600 hover:text-brand-600 transition">Kemitraan</a>
                    </div>
                </div>
            </div>

            <a href="{{ route('berita') }}" @click="mobileOpen = false" class="block px-3 py-3 text-slate-600 font-medium hover:bg-slate-50 hover:text-brand-600 rounded-md border-b border-slate-50">Berita</a>
